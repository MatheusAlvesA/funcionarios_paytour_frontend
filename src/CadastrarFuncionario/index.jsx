...
class CadastrarFuncionario extends React.Component {
	...
	cadastrar = async e => {
		...
		const r = await cadastrarFuncionario(payload);
		...
		if(r === true) {
			...
			Swal.fire({
				title: 'Concluído',
				text: `Novo funcionário cadastrado`,
				type: 'success',
	...
	prepararPayload = async () => {
		...
		const foto = await this.toBase64(this.state.foto);
	...
	validarCampos = () => {
		...
		if(!this.validarFoto(this.state.foto)) {
